...
int
playerdb_init(struct hall *self) {
    redis_initreply(&self->reply, 512, 0);
    return 0;
}

void
playerdb_fini(struct hall *self) {
...
int 
playerdb_send(struct service *s, struct player *pr, int type) {
...
void 
playerdb_process_redis(struct service *s, struct UM_REDISREPLY *rep, int sz) {
    struct hall *self = SERVICE_SELF;
