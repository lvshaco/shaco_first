...
static void
member_update_delay(struct module *s, struct gameroom *ro, struct player *m, uint64_t now) {
    struct room *self = MODULE_SELF;
    ...
    for (i=0; i<m->total_delay.sz; ++i) {
        ...
        if (delay->first_effect_time > sh_timer_now()) {
            continue;
        }
...
static struct player *
login(struct module *s, int source, uint32_t roomid, const struct tmemberdetail *detail) {
    struct room *self = MODULE_SELF;
...
static void
AI_update_move(struct module *s, struct player *pr, float elapsed) {
    struct AI_brain *brain = pr->brain;
    sh_trace("AI %u update move (%d, %f)", UID(pr), brain->dir, brain->speed);
    if (brain->dir == 0) {
        return;
    }
    ...
    if (abs(depth) >= 1) {
        ...
        sh_trace("AI %u update depth (%d, cur %d)", UID(pr), depth, pr->depth);
    } 
}
...
AI_focus_target(struct player *pr, const struct AI_target *target) {
    ...
    sh_trace("AI %u switch to focus (%u in %u,%u b %u)", UID(pr), 
            target->id, 
            target->w, 
            target->h, 
            target->block);
...
AI_can_pick(struct player *pr) {
    ...
    if (brain->target.id > 0) {
        ...
        sh_trace("AI %u pick target1 rate %d", UID(pr), rate);
        if (rate >= 100 ||
            rand()%100 <= rate) {
            return true;
        }
    } else if (brain->target2.id > 0) {
        sh_trace("AI %u pick target2 rate 100", UID(pr));
        return true;
    }
    return false;
...
static void
AI_pick_target(struct module *s, struct gameroom *ro, struct player *pr) {
    struct AI_brain *brain = pr->brain;
    struct AI_target *target = AI_current_target(brain);
    if (target) {
        sh_trace("AI %u rand pick target (%u in %u,%u)", UID(pr), 
            brain->target.id, 
            brain->target.w, 
            brain->target.h);
        if (AI_can_pick(pr)) {
            sh_trace("AI %u pick target %u ok", UID(pr), target->id);
    ...
    if (brain->target2.id > 0) {
        ...
        brain->status_time = 0;
        brain->status_intv = 0; 
        sh_trace("AI %u switch to move", UID(pr));
    }
}
...
static void
AI_press(struct module *s, struct player *pr, uint64_t now, int press_time) {
    struct AI_brain *brain = pr->brain;
    ...
    sh_trace("AI %u press begin", UID(pr));
}

static void
AI_main(struct module *s, struct gameroom *ro, struct player *pr) {
    struct room *self = MODULE_SELF;
    struct AI_brain *brain = pr->brain;
    assert(brain);
    ...
    sh_trace("AI %u level %d elapsed %f", UID(pr), brain->level, elapsed);
    int buff_value = 0; // todo
    ...
    if (trans) { 
        ...
        sh_trace("AI %u [trans] speed %f depth %d", UID(pr), brain->speed, pr->depth);
    ...
    if (brain->tick % 10 == 0) { 
        ...
        if (brain->press_time == 0) {
            sh_trace("AI %u press rand", UID(pr));
            ...
            if (press_time <= (int)(now - brain->press_time)) {
                brain->press_time = 0;
                sh_trace("AI %u press end", UID(pr));
            }
            goto AI_end;
        } 
    }
