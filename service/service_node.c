...
_block_read(int id, int *msgsz, int *err) {
    for (;;) {
        ...
        if (*err == 0) {
            if (buf.sz > 6) {
                void *msg = buf.ptr+6;
                int sz = sh_from_bigendian16((uint8_t*)buf.ptr) + 2;
                if (buf.sz >= sz) {
                    buf.ptr += sz;
                    buf.sz -= sz;
...
_send(struct remote *self, int source, int dest, int type, const void *msg, size_t sz) {
    ...
    if (no->connid != -1) {
        ...
        sc_error("Node %d has not connect", id);
        return 1;
    } 
}
...
_broadcast_node(struct service *s, int id) {
    ...
    for (i=0; i<NODE_MAX; ++i) {
        ...
        _vsend(self, SERVICE_ID, other->node_handle, "ADDR %d %s %u %s %u",
                self->myid, no->addr.naddr, no->addr.nport, no->addr.gaddr, no->addr.gport);
    }
...
_read(struct service *s, struct net_message *nm) {
    ...
    for (;;) {
        ...
        for (;;) {
            ...
            buf.ptr += msgsz;
            buf.sz  -= msgsz;
            uint16_t source = sh_from_bigendian16((uint8_t*)buf.ptr+2);
            uint16_t dest = sh_from_bigendian16((uint8_t*)buf.ptr+4);
